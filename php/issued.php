...
if ($filter === 'all') {
    $sql = "SELECT ROW_NUMBER() OVER (ORDER BY borrow_date) AS number,
             borrow.id, book_detail.title, 
             CONCAT(account_profile.firstname, ' ', 
             account_profile.lastname) AS borrower_name, 
    ...
    $sql = "SELECT ROW_NUMBER() OVER (ORDER BY borrow_date) AS number,borrow.id, 
            book_detail.title, 
